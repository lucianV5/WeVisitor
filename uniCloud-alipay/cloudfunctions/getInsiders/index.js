...
const businessMain = async (event, context) => {
  ...
  try {
    ...
    const res = await insidersCol.where(where).orderBy('createTime', 'desc').limit(500).get();
    let list = res.data || [];
    ...
    console.error('[getInsiders] error:', err);
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    console.error('[getInsiders] main error:', msg, err && err.stack ? err.stack : err);
