...
const businessMain = async (event, context) => {
  ...
  const res = await db.collection('insiderApplications').where(where).orderBy('createTime', 'desc').limit(200).get();
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    console.error('[getInsiderApplications] main error:', msg, err && err.stack ? err.stack : String(err));
