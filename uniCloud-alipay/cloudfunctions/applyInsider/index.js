...
const businessMain = async (event, context) => {
  ...
  const linked = await db.collection('insiders').where({ _openid: openid }).limit(1).get();
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    console.error('[applyInsider] main error:', msg, err && err.stack ? err.stack : String(err));
